  ...
  return (
    <section className="settings-section" aria-labelledby="performance-title">
      ...
      <details className="advanced-settings"><summary>高级设置</summary><div className="performance-grid">
        ...
        <Tooltip content="每保存一批包含多少条运单。实测推荐 50；20 更容易恢复，100 在本机稳定性不足。"><label>每批保存数量<select value={settings.networkBatchSize} onChange={(event) => setSettings({ ...settings, networkBatchSize: Number(event.target.value) as 20 | 50 | 100 })}><option value={20}>20</option><option value={50}>50（推荐）</option><option value={100}>100</option></select></label></Tooltip>
