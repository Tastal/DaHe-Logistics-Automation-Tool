...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Create a development-only form-encoded Chengfeng detail "
            "contract candidate from the current selected contract."
        ),
        allow_abbrev=False,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = rollover_live_detail_encoding_contract(
